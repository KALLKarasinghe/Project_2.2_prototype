...
const Navbar = () => {
  ...
  if (currentUser) {
    ...
    if (role === 'admin') {
      ...
      dashboardLink = '/agent';
    } else if (role === 'supplier' || role === 'company') {
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="p-2 space-y-1">
                      ...
                      {['admin', 'supplier', 'company', 'pharmacy', 'customer', 'medical agent', 'agent'].includes(currentUser.role?.toLowerCase()) && (
                        <Link to={dashboardLink} onClick={() => setUserMenuOpen(false)} className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-700 rounded-xl hover:bg-slate-50 hover:text-blue-600 transition-colors">
