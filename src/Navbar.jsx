                ...
                {userMenuOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="p-2 space-y-1">
                      {/* Dashboard Link */}
                      {['admin', 'supplier', 'company', 'pharmacy', 'customer', 'medical agent', 'agent'].includes(currentUser.role?.toLowerCase()) && (
                        <Link to={currentUser.role?.toLowerCase() === 'admin' ? '/admin' : currentUser.role?.toLowerCase() === 'pharmacy' ? '/pharmacy' : currentUser.role?.toLowerCase() === 'customer' ? '/customer' : currentUser.role?.toLowerCase().includes('agent') ? '/agent' : '/supplier'} onClick={() => setUserMenuOpen(false)} className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-700 rounded-xl hover:bg-slate-50 hover:text-blue-600 transition-colors">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                          My Dashboard
                        </Link>
                      )}
                      
                      {!['customer', 'medical agent', 'agent'].includes(currentUser.role?.toLowerCase()) && (
