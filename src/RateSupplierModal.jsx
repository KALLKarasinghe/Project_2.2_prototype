...
const RateSupplierModal = ({ isOpen, onClose, order, currentUser }) => {
  ...
  const stars = [];
  for (let i = 1; i <= 5; i++) {
    let btnClass = 'p-2 rounded-xl transition-all ';
    if (rating >= i) {
      btnClass += 'text-amber-400 bg-amber-50';
    } else {
      btnClass += 'text-slate-300 bg-slate-50 hover:bg-slate-100';
    }
    ...
    stars.push(
      <button
        ...
        className={btnClass}
      >
        <svg className="w-8 h-8" fill={iconFill} stroke="currentColor" viewBox="0 0 24 24">
