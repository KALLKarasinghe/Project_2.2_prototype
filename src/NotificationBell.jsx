...
const NotificationBell = () => {
  ...
  // show drop down header badge if any unread
  let headerBadge = null;
  if (unreadCount > 0) {
    headerBadge = <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full font-bold">{unreadCount} New</span>;
  }
    ...
    notifContent = (
      <div className="divide-y divide-slate-50">
        {notifications.map(notif => {
          let rowClass = 'p-4 hover:bg-slate-50 transition-colors cursor-pointer ';
          if (!notif.is_read) {
            rowClass += 'bg-blue-50/30';
          }
          
          let dotClass = 'w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ';
          if (!notif.is_read) {
          ...
          return (
            ...
            >
              <div className="flex gap-3">
                ...
                <div>
                  <p className={titleClass}>{notif.title}</p>
